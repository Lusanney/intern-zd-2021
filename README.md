...
- **Ticket Detail**: This page shows data of a single ticket, such as _subject_, _description_, _requester_id_, _status_. This page is ID-oriented, that means, if you navigate to a different ticket id page `/ticket/:id`, you will have a different ticket.
...
I assume that, my trial account is not expired, and the API call is always either successful or error with logical problems. Right now, there is no documentation on the error show trial expired on API calls. It is also to notice that, we only handle errors that API is unavailable, but it is not required to handle such trial expired case.
